...
static void RunBenchItersArg(Benchmark* b, int iters, int arg) {
  t0 = nsec();
  ns = 0;
  bytes = 0;
  items = 0;
...
void RunBench(Benchmark* b, int arg) {
  int iters, last;

  // Run once just in case it's expensive.
  iters = 1;
  RunBenchItersArg(b, iters, arg);
...
static bool match(const char* name, int argc, const char** argv) {
  if (argc == 1) return true;
  for (int i = 1; i < argc; i++) {
    if (RE2::PartialMatch(name, argv[i])) return true;
  }
  return false;
}
...
int main(int argc, const char** argv) {
  for (int i = 0; i < nbenchmarks; i++) {
    ...
    if (!match(b->name(), argc, argv)) continue;
    for (int arg = b->lo(); arg <= b->hi(); arg <<= 1) RunBench(b, arg);
  }
}
