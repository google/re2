...
def PrintGroup(name, codes):
  ...
  hexname = str(codecs.encode(bytes(name, 'ascii'), 'hex'), 'ascii')
  ugroup = "{ \"%s\", +1" % (hexname,)
  hexname = 'X'+hexname
  # if len(code16) > 0:
  #   PrintCodes("uint16_t", hexname+"_code16", code16)
  #   ugroup += ", %s_code16, %d" % (hexname, len(code16))
  # else:
  #   ugroup += ", 0, 0"
  if len(range16) > 0:
    PrintRanges("URange16", hexname+"_range16", range16)
    ugroup += ", %s_range16, %d" % (hexname, len(range16))
  else:
    ugroup += ", 0, 0"
  if len(range32) > 0:
    PrintRanges("URange32", hexname+"_range32", range32)
    ugroup += ", %s_range32, %d" % (hexname, len(range32))
  else:
    ugroup += ", 0, 0"
  ugroup += " }"
  return ugroup
