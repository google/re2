...
DFA::State* DFA::CachedState(int* inst, int ninst, uint flag) {
  ...
  // The elements of next_ are zero-initialised whereas the elements of
  // inst_ are default-initialised because we then copy them from inst.
  char* space = new char[mem];
  State* s = new (space) State;
  auto ptr = new (s->next_) std::atomic<State*>[nnext]();
  (void)ptr;  // USED(ptr)
