...
<<<<<<< HEAD   (708957 Tweak the comment on RE2::QuoteMeta().)
  if (nvec > static_cast<int>(ABSL_ARRAYSIZE(vec)))
=======
  if (nvec > 1 + re.NumberOfCapturingGroups())
    return false;
  if (nvec > static_cast<int>(arraysize(vec)))
>>>>>>> CHANGE (028429 Refuse to rewrite when MaxSubmatch() is too large.)
    return false;
...
bool RE2::Arg::parse_string_view(const char* str, size_t n, void* dest) {
  if (dest == NULL) return true;
